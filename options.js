...
document.addEventListener("DOMContentLoaded", async () => {
  // Made DOMContentLoaded async to await storage check
  ...
  const extensionVersionAboutElement = document.getElementById(
    "extensionVersionAbout",
  ); // Added for About tab
  const extensionBuildAboutElement = document.getElementById(
    "extensionBuildAbout",
  );
  ...
  const useKeystrokesCheckbox = document.getElementById(
    "useKeystrokesCheckbox",
  );
  ...
  function updateUI(
    ...
    if (messageArea) {
      ...
      messageArea.className =
        "message " +
        (message.toLowerCase().includes("error") ? "error" : "success"); // Use classes
    ...
    if (isUnlocked) {
      ...
      if (
        unlockSection &&
        !unlockSection.classList.contains("hidden") &&
        sitePasswordInput
      ) {
  ...
  function startCountdown(expiryTime) {
    ...
    const update = () => {
      ...
      if (expiryDisplay)
        expiryDisplay.textContent = `${minutes}:${seconds.toString().padStart(2, "0")}`;
    };
    update();
  ...
  function fetchAndUpdateStatus(displayMessage = "") {
    ...
    chrome.runtime.sendMessage({ action: "getExtensionStatus" }, (response) => {
      ...
      if (response) {
        ...
        updateUI(
          response.isUnlocked,
          response.unlockExpiryTime,
          displayMessage,
          response.currentTimeoutMinutes,
        );
  ...
  // Add this message listener in options.js
  // Place it right after the DOMContentLoaded event listener, but before its closing bracket

  // --- Listen for direct URL updates from background script ---
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "pendingRequestInfo" && request.pendingRequest) {
      console.log(
        "Options page received pending request info:",
        request.pendingRequest,
      );
      if (callingTabUrlDisplay && request.pendingRequest.url) {
        callingTabUrlDisplay.textContent = request.pendingRequest.url;
      }
    }
  });
  ...
  function fetchAndDisplayCallingTabUrl() {
    ...
    chrome.runtime.sendMessage({ action: "getCallingTabInfo" }, (response) => {
      if (chrome.runtime.lastError) {
        console.error(
          "Error getting calling tab info:",
          chrome.runtime.lastError.message,
        );
    ...
    try {
      ...
      if (passwordLengthInput) {
        passwordLengthInput.value =
          settingsData?.[PASSWORD_LENGTH_STORAGE_KEY] ?? 28;
      }

      // Apply Emoji Setting
      if (useEmojisCheckbox) {
        useEmojisCheckbox.checked =
          settingsData?.[USE_EMOJIS_STORAGE_KEY] ?? true;
      }
      // Apply Keystroke Setting
      if (
        useKeystrokesCheckbox &&
        typeof USE_KEYSTROKES_STORAGE_KEY !== "undefined"
      ) {
        useKeystrokesCheckbox.checked =
          settingsData?.[USE_KEYSTROKES_STORAGE_KEY] ?? true; // default is useKeystrokes
      }

      // Apply Timeout Setting Display & Slider Value
      const timeoutToDisplay =
        currentTimeout !== null
          ? currentTimeout
          : (settingsData?.[TIMEOUT_STORAGE_KEY] ?? 5);
  ...
  if (unlockButton && sitePasswordInput) {
    const performUnlock = () => {
      ...
      const secretEntropy =
        useKeystrokesCheckbox && useKeystrokesCheckbox.checked
          ? capturedKeystrokes.join(",")
          : sitePasswordInput.value.trim();
      ...
      chrome.runtime.sendMessage(
        { action: "unlockExtension", siteSecret: secretEntropy },
        (response) => {
          ...
          if (chrome.runtime.lastError || !response?.success) {
            ...
            updateUI(
              false,
              null,
              `Unlock Failed: ${response?.error || chrome.runtime.lastError?.message || "Unknown error"}`,
            );
            ...
            fetchAndUpdateStatus(response.message || "Extension unlocked!");
          }
        },
      );
    };
  ...
  if (lockButton) {
    lockButton.addEventListener("click", () => {
      ...
      lockButton.innerHTML = `<span id="statusIcon">⏳</span> Locking...`;
      chrome.runtime.sendMessage({ action: "lockExtension" }, (response) => {
        lockButton.disabled = false;
        lockButton.innerHTML = `<span id="statusIcon">🔒</span> Lock Now`;
        if (chrome.runtime.lastError || !response?.success) {
          console.error(
            "Lock failed:",
            chrome.runtime.lastError?.message || response?.error,
          ...
          fetchAndUpdateStatus(
            `Lock Failed: ${response?.error || chrome.runtime.lastError?.message || "Unknown error"}`,
          );
        } else {
          console.log("Lock successful.");
          fetchAndUpdateStatus(response.message || "Extension locked.");
        }
      });
    });
  }
  ...
  if (setLengthButton && passwordLengthInput) {
    setLengthButton.addEventListener("click", () => {
      ...
      if (isNaN(lengthValue) || lengthValue < 8 || lengthValue > 64) {
        if (lengthMessage) {
          lengthMessage.textContent = "Invalid length (must be 8-64).";
          lengthMessage.className = "message error";
          lengthMessage.style.display = "block";
        }
        return;
      }
      setLengthButton.disabled = true;
      setLengthButton.textContent = "Saving...";
      chrome.runtime.sendMessage(
        { action: "setPasswordLength", length: lengthValue },
        (response) => {
          const msg = chrome.runtime.lastError?.message || response?.error;
          if (msg || !response?.success) {
            console.error("Set length error:", msg);
            if (lengthMessage) {
              lengthMessage.textContent = `Error: ${msg || "Failed to save"}`;
              lengthMessage.className = "message error";
              lengthMessage.style.display = "block";
            }
          } else {
            ...
            if (lengthMessage) {
              lengthMessage.textContent = "Length saved.";
              lengthMessage.className = "message success";
              lengthMessage.style.display = "block";
            }
            setTimeout(() => {
              if (lengthMessage) lengthMessage.style.display = "none";
            }, 3000);
          }
          setLengthButton.disabled = false;
          setLengthButton.textContent = "Set Length"; // Re-enable button outside of timeout
        },
      );
    });
  }
  ...
  if (useEmojisCheckbox) {
    useEmojisCheckbox.addEventListener("change", () => {
      ...
      if (emojisMessage) emojisMessage.textContent = "Saving...";
      emojisMessage.className = "message";
      emojisMessage.style.display = "block";
      chrome.runtime.sendMessage(
        { action: "setUseEmojis", useEmojis: useEmojis },
        (response) => {
          const msg = chrome.runtime.lastError?.message || response?.error;
          if (msg || !response?.success) {
            console.error("Set emojis error:", msg);
            if (emojisMessage) {
              emojisMessage.textContent = `Error: ${msg || "Failed to save"}`;
              emojisMessage.className = "message error";
            }
            useEmojisCheckbox.checked = !useEmojis; // Revert on error
          } else {
            console.log("Set emojis successful.");
            if (emojisMessage) {
              emojisMessage.textContent = useEmojis
                ? "Emojis enabled."
                : "Emojis disabled.";
              emojisMessage.className = "message success";
            }
          }
          setTimeout(() => {
            if (emojisMessage) emojisMessage.style.display = "none";
          }, 3000);
        },
      );
    });
  ...
  if (timeoutSlider && timeoutValueDisplay) {
    ...
    timeoutSlider.addEventListener("change", () => {
      ...
      if (timeoutMessage) timeoutMessage.textContent = "Saving...";
      timeoutMessage.className = "message";
      timeoutMessage.style.display = "block";
      timeoutSlider.disabled = true; // Disable slider while saving

      chrome.runtime.sendMessage(
        { action: "setTimeoutValue", timeoutMinutes: minutes },
        (response) => {
          ...
          if (msg || !response?.success) {
            ...
            if (timeoutMessage) {
              timeoutMessage.textContent = `Error: ${msg || "Failed to save"}`;
              timeoutMessage.className = "message error";
            }
  ...
  if (
    ...
    useKeystrokesCheckbox.addEventListener("change", () => {
      ...
      if (keystrokesMessage) keystrokesMessage.textContent = "Saving...";
      keystrokesMessage.className = "message";
      keystrokesMessage.style.display = "block";
      // Use the renamed action name
      chrome.runtime.sendMessage(
        { action: "setUseKeystrokes", enabled: isEnabled },
        (response) => {
          const msg = chrome.runtime.lastError?.message || response?.error;
          if (msg || !response?.success) {
            console.error("Set keystroke error:", msg);
            if (keystrokesMessage) {
              keystrokesMessage.textContent = `Error: ${msg || "Failed to save"}`;
              keystrokesMessage.className = "message error";
            }
            useKeystrokesCheckbox.checked = !isEnabled; // Revert
          } else {
            console.log("Set keystroke successful.");
            if (keystrokesMessage) {
              keystrokesMessage.textContent = isEnabled
                ? "Keystroke inputs enabled."
                : "Keystroke inputs disabled.";
              keystrokesMessage.className = "message success";
            }
          }
          setTimeout(() => {
            if (keystrokesMessage) messageArea.style.display = "none";
          }, 3000); // Use messageArea
        },
      );
    });
  }
